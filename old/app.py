...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.H1(children=' '),
                     html.Button(id='submit-button', children='Submit', n_clicks_timestamp=-1),     
...
def update_figure(selected_year,selected_measure,selected_construct,timestmp):
    skip=0
    skipm=0
    skipc=0
    ...
    now = datetime.now()
    timenow = datetime.timestamp(now)-1
    if (timestmp/1000 < timenow) :
...
@app.callback(
    ...
    Input('measure-select', 'value'),
     Input('construct-select', 'value'),
     Input('submit-button', 'n_clicks_timestamp')])
def update_table(selected_year,selected_measure,selected_construct,timestmp):
    skip=0
    dff1=df[df['Pub Year'] < 2014]
    dff2=df[df['Pub Year'] < 2014]
    measure=selected_measure
